...
MMKV uses `mmap`, a Unix system call that maps files into memory. Since most heavy lifting is handed off to the operating system, and the library is basically reading / writing memory, it comes as no surprise that MMKV appears to perform the best among the four.
...
Despite its potential efficiency due to Protocol Buffers, we are surprised to find that DataStore perform noticeably worse than SharedPreferences. My friend published his results in this [blog post](https://juejin.cn/post/6931912030144167950). I also played with the problem a bit and can report similar results.


## Atomicity guarantee

After some study, I believe DataStore's relative slow performance is due to its atomicity guarantee. Unfortunately, a lot of articles fail to higlight this important feature that DataStore provides.
...
It is unlikely to print 10000 in the end. This concurrency anamoly is called *lost update*. Note that on the JVM, the `++` operator is actually two separate operations. What happens is that some later write **clobbers** the earlier write, since the counter state is shared and mutated in multiple concurrent coroutines. Lost update often occurs when application does a read-modify-write cycle to shared mutable state.

This particular counter issue can be solved on the JVM using an `AtomicInteger`. Kotlin has [a nice documentation](https://kotlinlang.org/docs/shared-mutable-state-and-concurrency.html) on how to solve the more general cases where no off-the-shelf atomic data structures are available.

If you implement this simple counter naively with SharedPreferences or other persistence mechanisms without atomicity guarantee, it won't work correctly. When we say SharedPreferences is thread-safe, it only means that the class has proper internal synchronization to prevent data structure corruption when its methods are called concurrently. It does not necessarily protect against *lost update* anamoly, which is about multiple operations that should be treated as a single unit.
...
I wrote a demo app which demonstrates that DataStore increments the counter correctly while SharedPreferences does not.
...
Note that in this example, both read and write methods are modeled as supend functions. This is necessary because readers and writers should block each other. If either a read or write operation is in process, other concurrent operations must wait until their request gets processed by the actor. The actor notifies the caller through `CompletableDeffered` after processing their messages.

In effect, Kotlin's actor uses channel under the hood to synchronize access to state. Channel is a synchronization primitive and synchronization comes at a cost. I did a cursory experiment that measures the overhead of using actors.
...
Note that we can only use property delegates in a class or in a file as top-level declarations. This means it's impossible to use this helper in a loop or in a function. Since we can still create mutiple class instances, the correct way is put these declarations in a file. Such API enforces developers to statically declare the DataStore they want to use in their project. It makes accidentally creating multiple DataStore instances on the same file much harder.
...
See [release notes](https://developer.android.com/jetpack/androidx/releases/datastore#1.0.0-alpha07) on Android Developers website.
...
It occurs to me that the Chinese Android community is rushing to replace Jetpack DataStore with MMKV. MMKV seems great, but it exposes only low level APIs. Without proper abstraction in place, people could easily mess up their code bases. 
...
Here are some interesting questions. I may return to this post and expand a bit more if I can reach at some conclusion.
