...
```
如果集合元素非常多，或者又需要经过很多操作符变换，这样每次变换都创建一个新集合存储中间结果，会有些浪费。
...
而使用协程和 Flow 的话，异步函数返回值不需要戴任何套子，数据流变换可以直接使用 `map`，更加自然。
...
- [KotlinConf 2019: Asynchronous Data Streams with Kotlin Flow](https://www.youtube.com/watch?v=tYcqn48SMT8)：
  听 Kotlin Team Lead, Kotlin 协程和 Flow 的主要设计者 Roman Elizarov 先生介绍 Flow 设计的精妙之处。
- 本文最后介绍的 Android 函数反应式的例子比较简陋，建议参考 Jake Wharton 的 [Managing State with RxJava](https://www.youtube.com/watch?v=0IKHxjkgop4)。
