...
我们在 Sequence builder 块中用一个无限循环，源源不断地计算序列中的元素，并通过 `yield` 将元素发送给消费者。Sequence 的消费者就是所谓的终端操作符（terminal operator）,比如 `forEach`，`sum` 等。
...
以 `forEach` 为例，每当我们在 `sequence` 块中 `yield` 一个新元素，便会将这个元素发送给 `forEach` 块进行处理，然后再回到 `sequence` 块中执行后续的逻辑，计算下一个值。而普通的 List 则会先计算出所有的元素存在内存中。正因为如此，我们可以用 Sequence 代表无限元素的序列。但是在消费数据的时候必须使用 `take` 中间操作符限制消费的数量。
...
```
假设我们想要找出第一个不为 null 的 `Feedback.message`， 即 `"Cool and clear function"`。
有下面几种方式：
...
```
如果集合元素非常多，或者又需要经过很多操作符变换，这样每次变换都创建一个新的集合存储中间结果会有些浪费。
...
这道力扣题目是这样：给你一个嵌套的整型列表。请你设计一个迭代器，使其能够遍历这个整型列表中的所有整数。整型列表由这个接口表示：
...
Flow 借助语言层面的 suspend 机制实现异步，而对比之下 RxJava 需要在流中的数据类型体现异步。比如使用 RxJava 做网络请求：
...
可以看到，借助 Kotlin 已有的 suspend 基础设施，实现一个支持异步的反应式数据流不过两三行。实际上，Kotlin 协程库的 Flow 实现经过抽丝剥茧，核心代码和我们的玩具实现并没有太多区别，但提供了两个额外的保证：上下文保存（Context preservation）和异常透明（Exception transparency）。
...
调用方因为要更新 UI，希望在主线程消费数据。由于我们在 androidx 提供的 `LifecycleScope` 上开启的协程内调用 suspend `collect` 方法，
而 LifecycleScope 的 CoroutineContext 指定了主线程为协程调度器，因此我们可以确定 collect 的块会在主线程执行。
