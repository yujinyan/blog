...
如果只需要将元素存进一个 List，那么可以用递归轻松解决这个问题：
...
拿到一个 List 以后也可以转成一个迭代器。但是迭代器应该是「懒」的，也就是说消费者一边消费数据，迭代器一边遍历数据源。这样的好处包括：

- 如果消费者只需要前几条数据，而数据源又很大，那么预先计算出整个 List 做了很多无用功。
- 消费者不用等待整个 List 计算完成后才进行下一步处理。通过并发处理可以加快整个数据处理流程的速度。

这样我们就不方便直接递归，需要自己手动维护一个栈，原本简单的代码变得复杂了许多（可以在[这里](/leetcode/flatten-nested-list-iterator/#solution)查看完整代码）。
...
[[tip | 💡]]
| 读者或许会想到，可以给 `walk` 函数传一个回调，这样也能实现边遍历边消费的效果。可以理解成 Sequence 就是对这一方式的封装。还记得 suspend 本质上是回调。
| Sequence builder 是一个（受限的）suspend 块，而 `yield` 是 suspend 函数。

## Flow：可以 suspend 的 Sequence

上面的例子都是纯计算的操作。实际场景中我们可能需要在中间或者终端操作符做一些耗时的操作（假设都封装成了 suspend 函数），比如调接口、从数据库读写数据：
...
```kotlin
yield("foo") -> search("foo") -> saveToDB(result of search "foo") ->
yield("bar") -> search("bar") -> saveToDB(result of search "bar")
```
