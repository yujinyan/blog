...
To subscribe to scroll events, I expected to find something to which we can attach a callback (like `RecyclerView.OnScrollListener`), or an event stream like a `Flow` of scrolled offsets. It turns out `ScrollableState` does take a lambda:
...
On the other hand, `ScrollState` used by `Modifier.verticalScroll` exposes a `ScrollState.value` attribute, which gives us the current scroll position. It isn't obvious, but we can subscribe to this value by using `snapshotFlow`.
