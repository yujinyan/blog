...
issueId: 61
discussionId: 174
description: 通过 Retrofit Call Adapter 和 Converter 打造舒适的 Retrofit 协程使用体验。
---
...
```kotlin
lifecycleScope.launch {
  val user = runCatching { retrofit.create<UserService>().getUser(1) } // highlight-line
...
另外，try catch suspend 函数的时候需要注意重新抛出 `CancellationException`，否则可能导致协程块无法及时取消。
详细可以查看 [JCIP Notes - Interruption and Cancellation](/posts/jcip-notes-interruption-cancellation/#comparison-with-kotlin-coroutines)。
...
一种方案是返回 nullable 的类型。这样可以利用 Kotlin 的空安全操作符 `?.`，`?:` 和 `!!`：
...
更加令人期待的是 Kotlin [计划让空安全操作符同样适用于 Result](https://github.com/Kotlin/KEEP/pull/244)，于是我们可以这样写：
...
<KotlinSection class="mt-8" currentSlug="kotlin-coroutine-retrofit" />