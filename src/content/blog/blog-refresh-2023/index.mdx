...
description: 2023年末翻新了这个博客，了解 Astro、Solid.js、Tailwind 等新技术。
...
原先博客使用 Gatsby 在编译时服务端渲染生成页面（SSG）。这一方案在客户端仍然需要重新 hydrate 整个 React App。对于个人博客这种内容为主的网站似乎有些重，不需要把整个页面跑在 React 里面，直接输出固定的 HTML 即可。对此，React 的回应是 React Server Component。RSC 依赖具体框架的实现，可以选择：
